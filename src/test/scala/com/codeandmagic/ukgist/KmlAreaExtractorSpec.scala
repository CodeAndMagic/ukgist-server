...
object KmlAreaExtractorFixture extends Mockito{
  ...
  val CITY_OF_LONDON_1_KML_ROW = mockOrBrokerRow
  ...
  def mockOrBrokerRow = {
    val row = mock[Row]
    row.bigInt(anyString) returns Some(1)
    row.string(anyString) returns Some(CITY_OF_LONDON_1_AREA_NAME)
    row.binaryStream(anyString) returns Some(new FileInputStream(CITY_OF_LONDON_1_KML_PATH))
    /*return*/ row
  }
